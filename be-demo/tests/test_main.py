...
class TestMain(unittest.TestCase):
    ...
    def test_login_success(self, mock_persistence, mock_services):
        ...
        self.assertEqual(
            ...
            LoginResponse(token="test_token"),
        )
    ...
    def test_login_invalid_email(self, mock_services):
        ...
        response = self.client.post(
            "/api/login", json={"email": "invalid-email", "password": "password"}
        )
    ...
    def test_try_luck_win(self, mock_persistence, mock_services):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            TryLuckResponse.model_validate(response.json()), TryLuckResponse(win=True)
        )
    ...
    def test_try_luck_lose(self, mock_persistence, mock_services):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            TryLuckResponse.model_validate(response.json()), TryLuckResponse(win=False)
        )
